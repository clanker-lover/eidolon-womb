...
class TestThreadCRUD(unittest.TestCase):
    ...
    def test_create_thread(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Hello")
        self.assertIsNotNone(thread.id)
        self.assertEqual(sorted(thread.participants), ["Brandon", "Eidolon"])
    ...
    def test_create_thread_with_initial_message(self):
        ...
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Greeting", msg)
        self.assertEqual(len(thread.messages), 1)
        self.assertEqual(thread.messages[0].content, "Hey there")

    def test_get_thread(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Test")
    ...
    def test_append_message(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Chat")
        msg = ThreadMessage(
            author="Eidolon",
            content="Hello Brandon",
            timestamp=datetime.now().isoformat(),
        )
    ...
    def test_append_message_reactivates_dormant(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Old chat")
...
class TestThreadListing(unittest.TestCase):
    ...
    def test_list_all(self):
        self.store.create_thread(["Brandon", "Eidolon"], "A")
        self.store.create_thread(["Brandon", "Psyche"], "B")
    ...
    def test_list_by_status(self):
        t1 = self.store.create_thread(["Brandon", "Eidolon"], "Active")
        t2 = self.store.create_thread(["Brandon", "Psyche"], "Dormant")
    ...
    def test_list_sorted_by_last_activity(self):
        t1 = self.store.create_thread(["Brandon", "Eidolon"], "Old")
        time.sleep(0.01)
        t2 = self.store.create_thread(["Brandon", "Psyche"], "New")
        threads = self.store.list_threads()
...
class TestThreadSearch(unittest.TestCase):
    ...
    def test_search_finds_matches(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Tech talk")
        now = datetime.now().isoformat()
        self.store.append_message(thread.id, ThreadMessage("Brandon", "I love Python", now))
        self.store.append_message(thread.id, ThreadMessage("Eidolon", "Python is great", now))
        self.store.append_message(thread.id, ThreadMessage("Brandon", "What about Rust?", now))
    ...
    def test_search_max_results(self):
        ...
        for i in range(10):
            self.store.append_message(
                thread.id, ThreadMessage("Brandon", f"keyword message {i}", now)
            )
    ...
    def test_search_case_insensitive(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Chat")
        now = datetime.now().isoformat()
        self.store.append_message(thread.id, ThreadMessage("Brandon", "Hello World", now))
...
class TestFindOrCreate(unittest.TestCase):
    ...
    def test_creates_when_none_exist(self):
        thread = self.store.find_or_create_thread(["Brandon", "Eidolon"])
        self.assertIsNotNone(thread.id)
        self.assertEqual(thread.status, "active")

    def test_finds_existing_active_thread(self):
        t1 = self.store.create_thread(["Brandon", "Eidolon"], "Existing")
        t2 = self.store.find_or_create_thread(["Eidolon", "Brandon"])
        self.assertEqual(t1.id, t2.id)

    def test_ignores_dormant_thread(self):
        t1 = self.store.create_thread(["Brandon", "Eidolon"], "Dormant")
        self.store.update_status(t1.id, "dormant")
        t2 = self.store.find_or_create_thread(["Brandon", "Eidolon"])
        self.assertNotEqual(t1.id, t2.id)

    def test_uses_custom_subject(self):
        thread = self.store.find_or_create_thread(
            ["Brandon", "Eidolon"], subject="Custom subject"
        )
...
class TestRecentActivity(unittest.TestCase):
    ...
    def test_returns_messages_from_others(self):
        ...
        now = datetime.now().isoformat()
        self.store.append_message(
            thread.id, ThreadMessage("Eidolon", "Hello Brandon!", now)
        )
        activity = self.store.get_recent_activity("Brandon")
    ...
    def test_excludes_own_messages(self):
        ...
        now = datetime.now().isoformat()
        self.store.append_message(
            thread.id, ThreadMessage("Brandon", "Hey", now)
        )
        activity = self.store.get_recent_activity("Brandon")
    ...
    def test_since_filter(self):
        ...
        activity = self.store.get_recent_activity("Brandon", since=cutoff)
        self.assertEqual(len(activity), 1)
        self.assertEqual(activity[0][1].content, "New message")


class TestBrandonAsParticipant(unittest.TestCase):
    """Brandon has no being_id — verify he works as a plain name participant."""
    ...
    def test_brandon_in_thread(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Chat")
        self.assertIn("Brandon", thread.participants)

    def test_brandon_messages(self):
        thread = self.store.create_thread(["Brandon", "Eidolon"], "Chat")
        now = datetime.now().isoformat()
        self.store.append_message(
            thread.id, ThreadMessage("Brandon", "Hey Eidolon", now)
        )
        self.store.append_message(
            thread.id, ThreadMessage("Eidolon", "Hello Brandon", now)
        )
        ...
        self.assertEqual(loaded.messages[0].author, "Brandon")
...
class TestCountActive(unittest.TestCase):
    ...
    def test_count_active_for_participant(self):
        ...
        self.store.create_thread(["Brandon", "Psyche"], "B")
        self.store.create_thread(["Eidolon", "Psyche"], "C")
        self.assertEqual(self.store.count_active(participant="Brandon"), 2)
