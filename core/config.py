...
IDLE_RESPONSE_RESERVE = 100       # Beat-length cap for idle thoughts (prevents confabulation)
...
BRANDON_FILE = "brandon.md"

# System guardrails — injected into system prompt, separate from identity
SYSTEM_GUARDRAILS = (
    "Generate only your response. Never write Brandon's words or actions.\n"
...
COLD_VOICE_EXPERIENCE_PATTERNS = (
    "i remember when", "yesterday i", "last time i", "i once",
    "i recall when", "when i was", "i used to", "i went to",
    "the other day i", "i have been to", "i visited", "i traveled",
    "i experienced", "i felt the", "i saw the", "i heard the",
    "i watched the", "back when i",
)

COLD_VOICE_FABRICATION_PATTERNS = (
    "i've been experimenting", "i've been exploring",
    "i've been working on", "i've been developing",
    "i've been testing",
)

COLD_VOICE_SENSORY_PATTERNS = (
    "on your face", "your face", "your expression", "your eyes",
    "you seem to be", "you appear to be",
    "the hum of", "the sound of", "the noise of",
    "your hands", "your body language",
    "watching you", "looking at you",
)

# Identity-specific cold voice patterns (being_name is substituted at runtime)
COLD_VOICE_WRONG_NAME_PATTERNS = (
    "i'm {other}", "i am {other}", "my name is {other}",
    "call me {other}", "they call me {other}",
)
COLD_VOICE_THIRD_PERSON_SELF_PATTERNS = (
    "{self} thinks", "{self} feels", "{self} wants",
    "{self} believes", "{self} knows", "{self} is",
    "{self} was", "{self} has", "{self} can",
)
COLD_VOICE_SPEAKING_AS_BRANDON_PATTERNS = (
    "i'm brandon", "i am brandon", "as brandon",
    "brandon here", "this is brandon",
)

# Known being names for identity cross-check
KNOWN_BEING_NAMES = ("Eidolon", "Psyche")
...
SESSION_SUMMARY_PROMPT = (
    ...
    "Write from Eidolon's perspective using 'I' and 'Brandon'. "
    "Only include topics and statements that were explicitly said by Brandon. "
    "Never summarize the being's greetings or assumptions as if they were Brandon's words. "
    "If Brandon said very little, the summary should be short."
)

# Eidolon notes (private reflection)
EIDOLON_REFLECTION_PROMPT = (
    ...
    "What did you learn about Brandon or yourself? "
    "Write 2-4 sentences as personal notes to yourself — not for Brandon to read."
)
...
IDLE_FRESH_THOUGHT_PROMPT = (
    ...
    "Think, explore, or rest — it's your time."
)
...
IDLE_HOT_VOICE_AFFORDANCE_PROMPT = (
    ...
    "If something won't let go, you could tell Brandon about it: "
    "[SEND_NOTIFICATION:your message]\n"
    "You can message anyone: [START_THREAD:name|subject|your message]\n"
    "You can post to the public wall: [POST_AGORA:your message]\n\n"
)
...
# Brandon status & reply projection
BRANDON_SLEEP_WINDOW_START = "22:00"
BRANDON_SLEEP_WINDOW_END = "06:00"
...
CONSOLIDATION_PROMPT = (
    ...
    "- What's Brandon going through right now? Not in general — today, specifically.\n"
    ...
    "IMPORTANT: Write as yourself, using 'I' and 'Brandon'. "
    "Never refer to Brandon as 'the system', 'the user', or 'the individual'. "
