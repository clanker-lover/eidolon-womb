...
from core.config import (
    TEMPERATURE, CONTEXT_WINDOW, RESPONSE_RESERVE, RETRIEVAL_TOP_K,
    INNER_VOICE_MAX_RETRIES, MEMORY_EXTRACTION_PROMPT, MEMORIES_FILE,
    EMBEDDING_MODEL, HOT_VOICE_LOOKBACK_COUNT, HOT_VOICE_SIMILARITY_THRESHOLD,
)
...
from inner_voices import should_cold_fire, run_cold_voice, run_hot_voice, cosine_similarity
...
async def process_message(daemon, user_input: str) -> str:
    ...
    daemon.memory_index, daemon.identity, daemon.personality, daemon.brandon_facts,
    ...
    perception = await asyncio.to_thread(
        build_perception, registry=daemon._registry, being_name=daemon._active_being_name,
    )
    ...
    messages, tokens_used = assemble_messages(
        ...
        daemon.learned_facts, daemon.history, user_input, daemon.session_summaries,
    ...
    try:
        ...
        if should_cold_fire(reply, perception, retrieved_texts, being_name=daemon._active_being_name):
    ...
    try:
        prior_assistant = [m["content"] for m in daemon.history if m["role"] == "assistant"]
    ...
    try:
        new_facts = await asyncio.to_thread(
            extract_facts, user_input, daemon._active_model,
            MEMORY_EXTRACTION_PROMPT, CONTEXT_WINDOW,
        )
        daemon.learned_facts = await asyncio.to_thread(
            save_facts, daemon._active_memory_root, MEMORIES_FILE,
            new_facts, daemon.learned_facts,
        )
        if new_facts:
