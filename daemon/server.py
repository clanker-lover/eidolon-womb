...
async def _handle_peek(daemon, writer: asyncio.StreamWriter) -> None:
    ...
    state_label = daemon.state.value
    ...
    response = {
        ...
        "last_cycle_time": daemon._last_cycle_time,
    }
        ...
        if first_line:
            ...
            if first_msg:
                ...
                if first_msg.get("type") == "thread_reply":
                    ...
                    await writer.wait_closed()
                    return
                if first_msg.get("type") == "command":
                    await _handle_command(
                        daemon, first_msg.get("command", ""), writer
                    )
                    writer.close()
                    await writer.wait_closed()
                    return
...
async def _handle_command(daemon, command: str, writer: asyncio.StreamWriter) -> None:
    ...
    if command == "sleep":
        ...
        if daemon.state == DaemonState.STASIS:
            await _send(
                daemon,
                writer,
                {"type": "status", "state": "stasis", "content": "Already in stasis."},
            )
            return
        daemon.state = DaemonState.STASIS
        await _send(
            daemon,
            writer,
            {
                "type": "status",
                "state": "stasis",
                "content": f"{daemon._active_being_name} is now in stasis (cycles paused).",
            },
        )

    elif command == "normal":
        if daemon.state == DaemonState.AWAKE_AVAILABLE:
            await _send(
                daemon,
                writer,
                {
                    "type": "status",
                    "state": daemon.state.value,
                    "content": "Already in normal mode.",
                },
            )
            return
        ...
        await _send(
            ...
            writer,
            {
                "type": "status",
                "state": daemon.state.value,
                "content": f"{daemon._active_being_name} resuming normal thought cycles.",
            },
        )
