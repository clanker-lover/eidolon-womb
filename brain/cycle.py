...
from brain.actions import (
    resolve_actions_async, looks_like_failed_request, FAILED_INTENT_FEEDBACK,
    extract_thread_intent, extract_dismiss_intent, execute_tag,
)
...
from config import (
    RETRIEVAL_TOP_K, RESPONSE_RESERVE, IDLE_RESPONSE_RESERVE,
    INNER_VOICE_MAX_RETRIES,
    IDLE_FRESH_THOUGHT_PROMPT, IDLE_CONTINUATION_PROMPT,
    IDLE_TOOL_CONTINUATION_PROMPT, IDLE_HOT_VOICE_AFFORDANCE_PROMPT,
    FATIGUE_INVOLUNTARY_SLEEP,
    CLOSURE_THOUGHT_COUNT, CLOSURE_MAX_CHARS,
    HOT_VOICE_LOOKBACK_COUNT, DEFAULT_SLEEP_HOURS, SLEEP_RECOVERY_MAP,
...
from core.patterns import (
    ...
    _SLEEP_CHOICE_PROMPT, _SLEEP_CHOICE_URGENT_PROMPT,
)
...
async def thought_cycle_inner(daemon) -> None:
    ...
    if daemon._pending_search_result:
        daemon._idle_history.append({"role": "user", "content": daemon._pending_search_result})
            ...
            for t, msg in activity[:5]:
                if msg.author == "System":
                    daemon._thread_store.mark_thread_read(
                        t.id, daemon._active_being_name,
                    )
    ...
    if (thread_notifs
        ...
        if candidate:
            ...
            logger.info(
                ...
                candidate["author"], daemon._active_being_name,
                candidate["thread_id"][:8], candidate["content"][:60],
            )

    perception = await asyncio.to_thread(
        build_perception, thread_notifs,
    ...
    if daemon._choosing_sleep:
        ...
        thread = daemon._thread_store.get_thread(notif["thread_id"]) if daemon._thread_store else None
    ...
    messages, tokens_used = assemble_messages(
        perception, daemon.identity, daemon.personality, daemon.brandon_facts,
        daemon.learned_facts, daemon._idle_history, thinking_prompt,
        daemon.session_summaries, retrieved_memories=retrieved,
    )
    ...
    if not daemon._choosing_sleep and daemon.fatigue >= FATIGUE_INVOLUNTARY_SLEEP:
        ...
        daemon._idle_history.append({"role": "user", "content": _SLEEP_CHOICE_URGENT_PROMPT})
    ...
    if daemon._choosing_sleep:
        ...
        logger.info("Sleep choice parsed: %dh %s (voluntary=%s) from: %s",
                    hours, sleep_info["label"], voluntary, reply[:80])
    ...
    reply = await resolve_actions_async(
        reply, daemon.generate_reply, messages,
    ...
    if not tags_fired and not daemon._choosing_sleep and not daemon._composing_thread_to:
        ...
        if curiosity:
            ...
            if (now - daemon._last_intent_search_time) >= cooldown_seconds:
                ...
                recent_thoughts = [msg["content"] for msg in daemon._idle_history[-6:] if msg.get("role") == "assistant"]
                intent_context = "\n---\n".join(recent_thoughts[-3:]) if recent_thoughts else reply[:300]
    ...
    if daemon._pending_thread_engagement:
        ...
        if tags_fired:
            ...
            logger.info("Explicit tags during engagement prompt — clearing engage state")
            ...
            logger.info("Thread engagement declined by %s for thread %s",
                        daemon._active_being_name, tid[:8])
        elif daemon._is_duplicate_thread_response(tid, reply):
            # Near-identical to a previous response — inject feedback, skip posting
            logger.info("Thread dedup blocked response from %s to thread %s (too similar to previous)",
                        daemon._active_being_name, tid[:8])
            ...
            logger.info("Thread engagement response: %s -> thread %s: %s",
                        daemon._active_being_name, tid[:8], result)
    ...
    if not engage_handled and daemon._composing_thread_to:
        if tags_fired:
            ...
            logger.info("Explicit tags fired during compose mode — clearing compose state")
            daemon._composing_thread_to = None
            daemon._composing_thread_topic = None
        elif _is_compose_decline(reply):
            logger.info("Compose declined for %s by %s",
                       daemon._composing_thread_to, daemon._active_being_name)
            daemon._composing_thread_to = None
            daemon._composing_thread_topic = None
        else:
            ...
            if topic:
                ...
                m = re.search(r'[.!?\n]', reply)
                subject = (reply[:m.start()] if m and m.start() > 0 else reply[:60]).strip()[:60]
            tag_arg = f"{target}|{subject}|{reply}"
            result = await asyncio.to_thread(execute_tag, "START_THREAD", tag_arg)
            logger.info("Compose flow created thread: %s -> %s: %s",
                       daemon._active_being_name, target, result)
            # Queue notification for Brandon threads
            if target.lower() == "brandon":
        ...
        if cycles_since_last >= 3:
            ...
            if intent:
                ...
                if action in ("respond", "message"):
                    ...
                    logger.info("Compose mode activated: %s -> %s (topic: %s)",
                               daemon._active_being_name, target, topic or "none")

    # 5e. Dismiss intent — being chooses not to engage with thread notifications
    if (thread_notifs and not tags_fired and not compose_injection
            and not daemon._composing_thread_to and not engage_handled):
        if extract_dismiss_intent(reply):
            for notif in thread_notifs:
                full_id = notif["thread_id"]
                result = await asyncio.to_thread(
                    execute_tag, "DISMISS_THREAD", full_id
                )
                logger.info("Dismiss intent: %s dismissed thread %s: %s",
                           daemon._active_being_name, full_id[:8], result)
    ...
    if voice_output and voice_name:
        ...
        daemon._idle_history.append({"role": "user", "content": f"{label}: {voice_output}"})
    ...
    logger.info(
        ...
        daemon.fatigue * 100, len(daemon._idle_history),
    )
    ...
    if (len(daemon._previous_thoughts) >= CLOSURE_THOUGHT_COUNT
            and all(len(t.strip()) < CLOSURE_MAX_CHARS
    ...
    if not should_close and len(daemon._previous_thoughts) >= CLOSURE_THOUGHT_COUNT:
        ...
        if rest_count >= 2:
            logger.info(
                ...
                rest_count, CLOSURE_THOUGHT_COUNT,
            )
            should_close = True
