...
1. **Inject Pending Results** — Carry forward search results from previous cycle
...
class DaemonState(Enum):
    ...
    STASIS = "stasis"                      # Paused by human, no cycles
```
...
- `AWAKE_AVAILABLE` → `STASIS`: Human pauses the daemon via dashboard
- `STASIS` → `AWAKE_AVAILABLE`: Human resumes via dashboard
